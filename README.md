...
병·의원 청구 사전점검 서비스를 표방하는 웹사이트를 퍼블리싱 연습을 위해 클론코딩한 프로젝트입니다.
HTML/CSS/JS 마크업 및 퍼블리싱 실력을 보여주기 위한 학습용 결과물이며, 실제 서비스가 아닙니다.

> 실존하는 서비스 UI를 학습 목적으로 모작했습니다. 회사명·사업자등록번호·대표자명·연락처 등은 모두 가상의 정보로 대체했습니다.
...
- 반응형 스타일은 공통 레이아웃(`css/common.css`)과 메인 페이지(`css/main.css`)에만 적용되어 있습니다. 나머지 페이지(`joinus.css`, `mypage.css`, `service.css`, `popup.css`, `board.css`)는 여전히 1280px 고정 폭 레이아웃이라 추가 작업이 필요합니다.
